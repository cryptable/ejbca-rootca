...
#########################################################################################
#
#    Copyright 2020 by David Tillemans <david.tillemans@cryptable.org>
#
#    This program is free software: you can redistribute it and/or modify
#    it under the terms of the GNU Lesser General Public License as published by
#    the Free Software Foundation, either version 2 of the License, or
...
database_name="ejbca"
database_driver="h2"
database_url="jdbc:h2:file:~/.ejbca/ejbcadb;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;"
...
WILDFLY_TAR_SHA256=54498e9c16b29c7f0a2cbab91edab6dfb9e2002259ee07c6d1247c23dc0cbb6c
...
# BEGIN H2 Database functions
init_h2() {
  cd $INSTALL_DIRECTORY || exit 1
  echo "Dropping all database tables in database"
  rm -rf ~/.ejbca/ejbcadb*
}

# END H2 Database functions


# BEGIN WildFly functions
wildfly_killall() {
...
# BEGIN EJBCA functions
create_keys_ontoken() {

pushd ${EJBCA_DIRECTORY}
  ant clean clientToolBox

  echo
  echo "HSM preparation"
  echo
  ...
  echo "Initialize Management CA Token"
  echo "Management CA Generate dummy key"
  # keytool -genkey -keyalg RSA -keysize 512 -keystore NONE -storetype PKCS11 -storepass $CA_TOKENPASSWORD -providerclass sun.security.pkcs11.SunPKCS11 -providerarg /etc/java-8-openjdk/security/utimaco-slot1.cfg -alias empty -dname "CN=empty"
  bin/pkcs11HSM.sh generate ${pkcs11_lib} ${test_key_size} empty ${pkcs11_slot} -password $CA_TOKENPASSWORD 
  echo "Management CA Generate test key"
  # keytool -genkey -keyalg RSA -keysize 512 -keystore NONE -storetype PKCS11 -storepass $CA_TOKENPASSWORD -providerclass sun.security.pkcs11.SunPKCS11 -providerarg /etc/java-8-openjdk/security/utimaco-slot1.cfg -alias test -dname "CN=test"
  bin/pkcs11HSM.sh generate ${pkcs11_lib} ${test_key_size} test ${pkcs11_slot} -password $CA_TOKENPASSWORD 
  echo "Management CA Generate default key"
  # keytool -genkey -keyalg RSA -keysize 2048 -keystore NONE -storetype PKCS11 -storepass $CA_TOKENPASSWORD -providerclass sun.security.pkcs11.SunPKCS11 -providerarg /etc/java-8-openjdk/security/utimaco-slot1.cfg -alias default -dname "CN=default"
  bin/pkcs11HSM.sh generate ${pkcs11_lib} ${default_key_size} default ${pkcs11_slot} -password $CA_TOKENPASSWORD 
  echo "Management CA Generate singing key"
  # keytool -genkey -keyalg RSA -keysize 4096 -keystore NONE -storetype PKCS11 -storepass $CA_TOKENPASSWORD -providerclass sun.security.pkcs11.SunPKCS11 -providerarg /etc/java-8-openjdk/security/utimaco-slot1.cfg -alias signing  -dname "CN=signing"
  bin/pkcs11HSM.sh generate ${pkcs11_lib} ${ca_key_size} signing ${pkcs11_slot} -password $CA_TOKENPASSWORD 
popd
...
#  cp "Download/mariadb-java-client-${MARIADB_CONNECTOR_VERSION}.jar" "wildfly/standalone/deployments/mariadb-java-client.jar" || exit 1
  ...
  # patch standalone.conf (not heavy server)
  ...
  # Add datasource
  wildfly_exec "data-source add --name=ejbcads --driver-name=\"h2\" --jndi-name=\"java:/EjbcaDS\" --connection-url=\"jdbc:h2:file:~/.ejbca/ejbcadb;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;\" --min-pool-size=5 --max-pool-size=150 --pool-prefill=true --prepared-statements-cache-size=50 --share-prepared-statements=true --transaction-isolation=TRANSACTION_READ_COMMITTED --use-ccm=true --user-name=\"sa\" --password=\"sa\" "
  wildfly_exec ":reload"
...
cat <<EOF > ejbca-custom/conf/catoken.properties
...
sharedLibrary=${pkcs11_lib}
slotLabelType=SLOT_NUMBER
slotLabelValue=${pkcs11_slot}
...
# >>>> NTS-Changes
ca.keystorepass=${CA_TOKENPASSWORD}
...
ca.tokenpassword=${CA_TOKENPASSWORD}
...
# >>>> NTS-Changes
ca.tokenproperties=${EJBCA_DIRECTORY}/conf/catoken.properties
...
cryptotoken.p11.lib.128.name=${pkcs11_name}
cryptotoken.p11.lib.128.file=${pkcs11_lib}
...
if [ -f /etc/redhat-release ]; then
  ...
  PKG_INSTALL="yum install tar unzip java-11-openjdk-devel ant psmisc mariadb bc patch"
  BASE_OS=RHEL
else if [ -f /etc/debian_version ]; then
  echo "found Debian/Ubuntu"
  PKG_INSTALL="apt install unzip openjdk-11-jdk-headless ant ant-optional psmisc bc patch"
  BASE_OS=UBUNTU
  else
